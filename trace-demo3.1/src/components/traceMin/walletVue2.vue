    ...
    <div class="wallet"
      ...
      <div class="walletTop">
        ...
        <div class="wa2">
          ...
          <span class="tag" v-show="tagShow" @dblclick="editTag">{{wallData.walletName2}}</span>
    ...
    export default {
      ...
      methods:{
        ...
        walletClick(id){
          ...
          that.ajax({
            ...
            if(res.data.code == "0000"){
              ...
              if(arr.indexOf(that.wallData.walletId) != -1){
                that.wallData.walletName2 = arr2[arr.indexOf(that.wallData.walletId)]
              }else {
                that.wallData.walletName2 = that.wallData.walletName.replace('实体',"钱包");
              }
        ...
        getTag(id){
          let that = this;
          that.tagShow = true;
          that.wallData.walletName2 = that.tagVal;
          if(that.Cookies.get('exchangeId2') == "" || that.Cookies.get('exchangeId2') == undefined){
            that.Cookies.set('exchangeId2',id);
          }else {
            let arr = that.Cookies.get('exchangeId2').split(",");
            arr.push(id);
            let str = arr.join(",");
            that.Cookies.set('exchangeId2',str);
          }
          if(that.Cookies.get('exchangeTag2') == "" || that.Cookies.get('exchangeTag2') == undefined){
            that.Cookies.set('exchangeTag2',that.tagVal);
          }else {
            let arr2 = that.Cookies.get('exchangeTag2').split(",");
            arr2.push(JSON.parse(JSON.stringify(that.tagVal)));
            let str2 = arr2.join(",");
            that.Cookies.set('exchangeTag2',str2);
          }
          // that.Cookies.set('exchangeTag2',that.tagVal);
      ...
      filters:{
        ...
        tagName(val){
          if(vm.Cookies.get('exchangeTag2') != ''){
            if(val.indexOf(vm.Cookies.get('exchangeId2')) != -1){
              console.log(1111)
              let tt = vm.Cookies.get('exchangeTag2');
              return tt;
            }else {
